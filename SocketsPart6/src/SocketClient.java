...
public class SocketClient {
     ...
     * 
     * int port = -1; try { // not safe but try-catch will get it port =
     * Integer.parseInt(args[0]); } catch (Exception e) {
     * log.log(Level.INFO,"Invalid port"); } if (port > -1) { try (SocketClient
     * client = new SocketClient();) { client.connect("127.0.0.1", port);
     * client.start(); } catch (IOException e) { e.printStackTrace(); } } }
     */
