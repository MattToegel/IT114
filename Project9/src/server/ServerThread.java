...
public class ServerThread extends Thread {
	...
	protected boolean sendAttackStatus(Point coords, int markerType) {
		Payload p = new Payload();
	...
	protected boolean sendRemainingShipCount(String clientName, int ships) {
		Payload p = new Payload();
		p.setPayloadType(PayloadType.SHIP_COUNT);
		p.setClientName(clientName);
		p.setNumber(ships);
		return sendPayload(p);
	}
	protected boolean sendPhaseUpdate(GameState placement) {
		Payload p = new Payload();
		p.setPayloadType(PayloadType.PHASE_CHANGE);
		p.setClientName(clientName);
		p.setNumber(placement.ordinal());
		return sendPayload(p);
	}
	protected boolean sendReset(String winner) {
		Payload p = new Payload();
		p.setPayloadType(PayloadType.RESET_GAME);
		p.setClientName(winner);
		return sendPayload(p);
	}
	...
	private void processPayload(Payload p) {
		...
		case PLACE_SHIP:
			...
			break;
		case ATTACK:
			currentRoom.attack(p.getPoint(), this);
			break;
		default:
