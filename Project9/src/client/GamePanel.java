...
public class GamePanel extends BaseGamePanel implements Event {
	...
	private Countdown turnCountdown;
	Image icon;
	...
	public synchronized void draw(Graphics g) {
		...
		if(icon != null) {
			g.drawImage(icon, 0, 0, gameAreaSize.width, gameAreaSize.height,this);
		}
		...
		if(gameState == GameState.TURNS && turnCountdown != null) {
			g.setColor(Color.WHITE);
			g.setFont(new Font("Monospaced", Font.PLAIN, 12));
			if (myPlayer != null) {
				g.drawString(turnCountdown.getTimeMessage(), (int)(gameAreaSize.width * .5f), (int)(gameAreaSize.height * .1f));
			}
		}
	}
	...
	private synchronized void clearShips() {
	...
	private void sendPlaceShip(MouseEvent e) {
		if (placedShips < maxShips) {
			if(e.getPoint().x >= 0 && e.getPoint().x <= gameAreaSize.width && e.getPoint().y >= 0 && e.getPoint().y <= gameAreaSize.height) {
	...
	public void onResize(Point p) {
		...
		System.out.println("Resized: " + this.getSize());
		this.invalidate();
		this.repaint();
	}
	...
	public void onCanAttack(String client, int attacks) {
		...
		if(myPlayer.getAttacks() > 0) {
			turnCountdown = new Countdown("Time Remaing", 30, (x)->{
				if(myPlayer.getAttacks() > 0) {
					SocketClient.INSTANCE.sendAttack(new Point(-1000,-1000));
					myPlayer.setAttacks(0);
				}
			});
		}
	}
	...
	@Override
	public synchronized void onReset(String winner) {
