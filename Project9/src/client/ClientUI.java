...
public class ClientUI extends JFrame implements Event {
	...
	void addClient(String name) {
		User u = new User(name,"<font color='purple'>" + name + "</font>");
	...
	public void onCanAttack(String client, int attacks) {
		System.out.println(client + " " + attacks);
		// TODO Auto-generated method stub
		if(username.equals(client) && attacks > 0) {
			self.addMessage("It's your turn, you have " + attacks + " attack(s) remaining");
			
		}
		Iterator<User> iter = users.iterator();
		while (iter.hasNext()) {
			User u = iter.next();
			if (u.getName().equals(client)) {
				u.isMyTurn(attacks>0);
				break;
			}
	...
	public void onShipCount(String client, int ships) {
		...
		while (iter.hasNext()) {
			User u = iter.next();
			if (u.getName().equals(client)) {
				//u.updateNameDisplay(String.format("%s: <b>%s</b>", client, ships));
				u.setShips(ships);
				break;
			}
	...
	public void onPhaseChange(GameState phase) {
		// TODO Auto-generated method stub
		self.addMessage("PHASE: " + phase.toString());
		if(phase == GameState.PLACEMENT) {
			shipPlacement = new Countdown("Place your ships", 15, null);
			Thread t = new Thread() {
				@Override
				public void run() {
					int c = 15;
					while(c > 0) {
						if(shipPlacement != null) {
							self.addMessage(shipPlacement.getTimeMessage());
						}
						c--;
						try {
							Thread.sleep(1000);
	...
	@Override
	public void onReset(String winner) {
		Iterator<User> iter = users.iterator();
		while (iter.hasNext()) {
			User u = iter.next();
			u.setShips(0);
		...
		self.addMessage(winner + " won this round!");
		self.addMessage("Next round eligible in about 10 seconds");
		
		Thread t = new Thread() {
			@Override
			public void run() {
				int c = 10;
				Countdown nextRound = new Countdown("Next Round", 10, (x)-> {
					
				});
				while(c > 0) {
					if(nextRound != null) {
						self.addMessage(nextRound.getTimeMessage());
					}
					c--;
					try {
						Thread.sleep(1000);
