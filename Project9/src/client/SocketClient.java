...
public enum SocketClient {
	...
	private void receiveShipCount(String client, int ships) {
		Iterator<Event> iter = events.iterator();
		while (iter.hasNext()) {
			Event e = iter.next();
			if (e != null) {
				e.onShipCount(client, ships);
			}
		}
	}
	private void receivePhaseChange(GameState phase) {
		Iterator<Event> iter = events.iterator();
		while (iter.hasNext()) {
			Event e = iter.next();
			if (e != null) {
	...
	private void receiveReset(String winner) {
		Iterator<Event> iter = events.iterator();
		while (iter.hasNext()) {
			Event e = iter.next();
			if (e != null) {
	...
	private void processPayload(Payload p) {
		...
		case SYNC_GAME_SIZE:
			...
			break;
		case PLACE_SHIP:
			receiveShipPlacement(Integer.parseInt(p.getMessage()), p.getId(), p.getPoint().x, p.getPoint().y, p.getNumber());
			break;
		case ATTACK:
			receiveAttackStatus(p.getNumber(), p.getPoint().x, p.getPoint().y);
			break;
		case SHIP_STATUS:
			receiveShipStatus(p.getId(), p.getNumber());
			break;
		case ATTACK_RADIUS:
		...
		case CAN_ATTACK:
			...
			break;
		case SHIP_COUNT:
			receiveShipCount(p.getClientName(), p.getNumber());
			break;
		case PHASE_CHANGE:
			receivePhaseChange(GameState.values()[p.getNumber()]);
			break;
		case RESET_GAME:
			receiveReset(p.getClientName());
			break;
		default:
