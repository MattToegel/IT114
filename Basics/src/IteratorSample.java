...
public class IteratorSample {
	public static void main(String[] args) {
		...
		/*
		 * Iterator<String> iter = months.iterator(); while (iter.hasNext()) { String
		 * month = iter.next();// we must call this to get the value at the current
		 * iteration if (month.indexOf("M") > -1) { iter.remove(); }
		 * System.out.println("Month: " + month); } System.out.
		 * println("You may think it didn't remove the values based on the output. Check again."
		 * ); iter = months.iterator(); while (iter.hasNext()) { String month =
		 * iter.next();// we must call this to get the value at the current iteration
